...
| 维度 | RAGFlow | 我们（retrieval_metadata.py，待接线） |
|---|---|---|
...
## 3. 本项目现状（2026-09-17 代码核查）

```mermaid
flowchart TD
    Q["question"] --> QA["QAService.query()"]
    QA --> HR["HybridRouter.retrieve()"]
...
| 5 | `QueryRewriteService` 已实现（严格 JSON、轮次分组、token 预算、注入防护），**未接入** `QAService` | `backend/src/apps/services/query_rewrite.py`；`qa_service.py` 无引用 |
...
def __init__(self, ..., metadata_generator=None):
    self._metadata = metadata_generator or RetrievalMetadataGenerator.from_env()
...
- `RetrievalMetadataGenerator.from_env()` 本身就是成本开关：不配置 `MVP_METADATA_LLM_*` → 无 LLM 客户端 → 自动走规则回退（文档名/章节加权短语 + 模板问题），**零 API 成本仍然产出全部字段**。默认部署可以只对业务/demo 库（几百子块量级）开启 LLM 版。
...
### 4.4 P1-A：查询侧关键词提取（合并进改写，一次 LLM 调用）
...
接线位置：`QAService.query()` 开头接入 `QueryRewriteService`（计划内 P1），随后：
...
| ES 层文本准入门槛（must+msm） | 与我们"独立软融合"是两种取舍；RAGFlow 靠级联回退补偿门槛的误杀。我们保持软融合，等 Weighted RRF A/B（计划 P0）一起评测后统一决策 |
...
|---|---|---|---|---|---|---|
| 1 | 索引侧关键词+问题生成 | `auto_keywords/auto_questions` + LLM 缓存 + 限流 | 生成器已写、**未接线** | ✅ 借鉴 | **P0** | §4.1；决定 BM25 上限，成本最低收益最直接 |
| 2 | BM25 增强字段（7 字段 boost） | `query_fields` 表（§2.1） | 只查 3 个原始字段 | ✅ 借鉴 | **P0** | §4.2；与 #1 配套，先 standard 版零重建 |
| 3 | `minimum_should_match` | 0.3 常规 / 0.1 回退 / vw≥0.8 时关闭 | 无 | ✅ 借鉴 | P0 起步、参数扫描定值 | 防泛词噪声准入；中文单字下需评测防误杀 |
| 4 | 空结果级联回退 | msm 降级→阈值放宽→纯向量 | 无 | ✅ 借鉴 | P1 | §4.5；与 `no_evidence` 协议衔接 |
| 5 | 查询侧 LLM 关键词抽取（追加式） | `keyword_extraction` 逗号拼接 | 无 | ✅ 借鉴（合并进改写单次调用） | P1 | §4.4；依赖 #5 查询改写接入（计划内） |
| 6 | 多轮查询改写 | `full_question` 纯文本+全量历史 | `QueryRewriteService` 更严格，未接入 | ✅（已加强，只需接线） | P1（计划内） | 我们版本有 JSON 校验/预算/防注入 |
...
实施本文 P0 后，回写 `docs/plan/README.md`：

1. "算法层"表 `BM25 检索` 行：现状从"搜索文档名、章节和正文"更新为包含增强字段；"standard analyzer 对中文可能不足"的 P1 判断不变，但补充"两侧一致性已保证，问题只在粒度"。
2. "后端层"表 `查询改写` 行：P1 接入时按 §4.4 的"单次调用双输出"实施。
3. "算法层"表 `Embedding 输入` 行的 P1 消融不受本文影响（向量通道仍只嵌子块正文/改写后问句）。
4. 新增事实：`RetrievalMetadataGenerator` 接线前属于"未进入主链的组件"，接线后更新该行状态。

## 7. 面试口径（一段话）

> "用户问法和原文不一致是 vocabulary mismatch，关键词抽取还有非确定性问题。RAGFlow 的解法是三段式：索引侧入库时让 LLM 为每个块生成关键词和'可能的用户问法'（带缓存，查询时零 LLM、结果确定），查询侧把 LLM 抽的关键词**追加**到原问题后面；真正匹配时不是逐词查询，而是把所有词、同义词、细粒度子词组成**一个加权布尔表达式**，一次 `query_string` 交给 ES，靠 IDF 让'计算机/网络'这类泛词权重趋零、靠 `minimum_should_match≈30%` 控制准入，重排阶段再用 token 相似度按字段放大（问题字段 ×6）。空了还有 msm 降级到纯向量的级联回退。我的项目 ES 同栈，借鉴了索引侧生成+增强字段+追加式抽词，但把两次 LLM 调用合并成改写时的一次调用，且向量通道只用干净问句、不拼关键词——这点我认为比 RAGFlow 的做法更干净。"
